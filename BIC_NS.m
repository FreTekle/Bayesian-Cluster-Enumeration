...
% This function implements BIC_N as a wrapper around the K-means algorithm
...
% Inputs:
%        data - a matrix that contains the data set with diamensions
%        num_features x num_samples_total
...
function [data_term, penalty_term, BIC] = BIC_NS(data, num_features, num_samples_total, spec_num_cluster, est_centroids)
...
zeta = num_features + 1; % number of estimated parameters per cluster
...
penalty_term = -zeta/2*sum(log(est_num_samples_cluster));
